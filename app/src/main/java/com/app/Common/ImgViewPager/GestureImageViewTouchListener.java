	...
	private void startZoom(MotionEvent e) {
		...
		if (image.isLandscape()) {
			if (image.getDeviceOrientation() == Configuration.ORIENTATION_PORTRAIT) {
				...
				// System.out.println("isLandscape scaledHeight=" + scaledHeight + " " + canvasHeight);
				if (Math.abs(scaledHeight - canvasHeight) < 2) {
				...
				// System.out.println("isLandscape scaledWidth=" + scaledWidth + " " + canvasWidth);
				if (Math.abs(scaledWidth - canvasWidth) < 2) {
					zoomTo = currentScale * 4.0f;
					zoomAnimation.setTouchX(e.getX());
					zoomAnimation.setTouchY(e.getY());
			...
			if (image.getDeviceOrientation() == Configuration.ORIENTATION_PORTRAIT) {
				int scaledHeight = image.getScaledHeight();
				// System.out.println("not isLandscape scaledHeight=" + scaledHeight + " " + canvasHeight);
				if (Math.abs(scaledHeight - canvasHeight) < 2) {
					zoomTo = currentScale * 4.0f;
					zoomAnimation.setTouchX(e.getX());
					zoomAnimation.setTouchY(e.getY());
				...
				int scaledWidth = image.getScaledWidth();
				// System.out.println("not isLandscape scaledWidth=" + scaledWidth + " " + canvasWidth);
				if (Math.abs(scaledWidth - canvasWidth) < 2) {
					zoomTo = currentScale * 4.0f;
					zoomAnimation.setTouchX(e.getX());
					zoomAnimation.setTouchY(e.getY());
